...
interface IBeIDServiceCallback {

	void smartCardReaderAttached();
	
	void smartCardReaderDetached();

	// return values: see BeIDServiceConstants
	int eIDCardInserted();
	
	void eIDCardRemoved();
	
	int eIDIdentity(in Identity identity);
	
	int eIDAddress(in Address address);
	
	int eIDPhoto(in byte[] photo);
}
